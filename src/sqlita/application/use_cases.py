...
from sqlita.application.dto import (
...
from sqlita.domain.events import (
    ParsingJobCompleted,
    ParsingJobStarted,
    SourceUnitParsed,
    SourceUnitParsingFailed,
)
from sqlita.domain.model import ParseOutcome, ParseStatus, ParsingJob, SourceUnit
from sqlita.domain.ports import (
    Clock,
...
class ParsingJobService:
    ...
    def parse_directory(self, command: ParseDirectoryCommand) -> ParsingJobReportDTO:
        source_units = tuple(self.source_repository.list_sql_sources(command.root_path))
