...
from sqlita.domain.errors import InputValidationError, SourceAccessError
from sqlita.domain.model import SourceUnit, SourceUnitId
from sqlita.domain.ports import SourceRepository
...
class FileSystemSourceRepository(SourceRepository):
    def load_file(self, path: str) -> SourceUnit:
        ...
        if source_path.suffix != ".sql":
            raise InputValidationError(f"expected a .sql file, got: {source_path}")

        return self._load_source_unit(source_path)

    def list_sql_sources(self, root_path: str) -> tuple[SourceUnit, ...]:
        ...
        source_paths = tuple(sorted(path for path in root.rglob("*.sql") if path.is_file()))
        if not source_paths:
            raise InputValidationError(f"no .sql files found under: {root}")
